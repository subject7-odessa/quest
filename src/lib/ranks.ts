...
// Título baseado no condicionamento FÍSICO REAL (média de Força,
// Velocidade e Resistência) — não no nível/XP. É por isso que alguém que
// chega no site já treinando pode entrar como "Atleta" no dia 1, mesmo
// com Nível 1 de XP (o nível mede disciplina/engajamento no app; o
// título mede o corpo de verdade).
export function titleForPhysicalAverage(avgPoints: number): string {
  const rank = rankForPoints(avgPoints).name;
  if (["EX", "Master", "Legend", "Myth", "Transcendent", "Infinite"].includes(rank)) return "Lenda";
  if (["S", "SS", "SSS"].includes(rank)) return "Elite";
  if (rank === "A") return "Monstro";
  if (rank === "B") return "Atleta";
  if (rank === "C") return "Guerreiro";
  if (rank === "D") return "Iniciante";
  return "Sedentário";
}
