...
import { createMission, toggleMissionActive, toggleBanUser, resetRankingXp, resetAllGameData } from "@/app/actions/admin";
...
export function AdminClient({ missions, users }: { missions: Mission[]; users: Profile[] }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <button
        onClick={() => {
          if (confirm("Isso apaga TODO o progresso (fichas, testes, exercícios, missões, conquistas) de TODOS os usuários. As contas de login continuam existindo. Confirma?")) {
            if (confirm("Tem certeza mesmo? Essa ação não pode ser desfeita.")) {
              startTransition(() => resetAllGameData());
            }
          }
        }}
        className="self-start border border-danger bg-danger/10 px-4 py-2 text-xs font-bold text-danger hover:bg-danger hover:text-void"
      >
        ⚠ RESETAR BANCO DE DADOS INTEIRO
      </button>
    </div>
  );
}
